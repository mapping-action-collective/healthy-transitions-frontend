...
function MapPage({ listings, listingMetadata, listingCities, listingCategories }) {
  const [ searchParams, ] = useSearchParams()
  const [ search, setSearch ] = useState()

  const { listingCategoryIcons } = listingMetadata
    ...
    <Segment as="nav" id="map-nav" color="black" basic vertical inverted>
      ...
      <Form size="tiny" className="container">
        <Grid>
          ...
          {/* Search  */}
          <Grid.Column as={Form.Input} width={12} tabIndex="1" 
            placeholder="Search" 
            action={{icon: "search"}} 
